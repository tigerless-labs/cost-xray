"""Smoke test for the Textual shell (tui_app). Skipped when textual isn't installed —
the interactive frontend is an optional extra (`pip install -e '.[tui]'`); its data/logic
is covered framework-free by test_drill.py and test_tui.py.

The cost/context panels are now drillable bordered **tables** (one Rich Table each, so the
$ columns stay column-aligned at any drill depth) rather than free functions, so we drive the
real mounted widgets via Textual's `run_test` (wrapped in asyncio.run — no pytest-asyncio dep)."""
...
_CAT = {  # one category's $ split (matches _agg's fields)
...
def test_q_quits_from_home_and_detail(monkeypatch, tmp_path):
    # The request: pressing `q` actually exits. The bug was a bare `quit` action, which Textual
    # resolves against the SCREEN namespace (no action_quit there) and silently drops. Drive `q`
    # through the real key path on both screens and assert the app received its exit.
    ...
    async def go():
        ...
        async with app.run_test() as pilot:
            ...
            await pilot.press("q")                            # q on Detail must quit the whole app
            await pilot.pause()
        ...
        async with app.run_test() as pilot:
            await app.workers.wait_for_complete()
            assert isinstance(app.screen, HomeScreen)
            await pilot.press("q")                            # q on Home must quit too
            await pilot.pause()
            assert app._exit is True
...
def test_home_groups_agent_project_session_and_hides_noise(monkeypatch, tmp_path):
    # Home is the same DrillTable as Detail: agent → project → session rows in ONE Rich table,
    # built from the per-agent ROLLUP (one read/agent). Agent/project rows are totals-only; only a
    # session leaf drills in (posts Picked). `unknown` agent + $0 hidden.
    from cost_xray import tui
    from cost_xray.tui_app import DetailScreen, HomeScreen, XrayApp
        ...
        str(root / "claude"): {
            "sessions": {
                ...
                "cccccccc3333": {"project": "/home/u/web", "name": "quota", "bill": 0.0001, "nt": 1,
                                 "cached": 0, "ci": 1, "tokens": 1, "mtime": 1.0},   # $0 → hidden
            },
            # precomputed totals (as the materializer writes them; $0 session excluded)
            "projects": {"/home/u/web": {"bill": 4.0, "tokens": 200, "cached": 140, "ci": 200,
                                         "nt": 30, "n_sessions": 2}},
            "totals": {"bill": 4.0, "tokens": 200, "cached": 140, "ci": 200, "nt": 30, "n_sessions": 2},
        },
        str(root / "unknown"): {"sessions": {  # whole agent skipped before this is even read
    ...
    async def go():
        ...
        async with app.run_test() as pilot:
            ...
            assert "unknown" not in home._agent_nodes              # unknown agent hidden
            assert set(home._sess_nodes) == {"aaaaaaaa1111", "bbbbbbbb2222"}  # $0 + unknown hidden

            pnode = home._proj_nodes[("claude", "/home/u/web")]
            assert "web" in pnode.label and pnode.data == {}       # project row = totals-only (no leaf data)
            assert "$4.00" in pnode.cols and pnode.expandable      # 1+3, drillable group
            assert len(home._proj_sids[("claude", "/home/u/web")]) == 2

            anode = home._agent_nodes["claude"]
            assert "$4.00" in anode.cols and anode.data == {} and anode.expanded   # agent expanded by default

            # every row — agent, project, session — carries the same count of numeric cells, so they
            # slot into the same fixed-width columns (this is what keeps them aligned at any depth)
            ncol = len(home._table.columns) - 1
            for n in (anode, pnode, *pnode.children):
                assert len(n.cols) == ncol

            # newest-activity-first: bbbb (mtime 3) before aaaa (mtime 2) under the project
            order = [n.data["sid"] for n in pnode.children]
            assert order == ["bbbbbbbb2222", "aaaaaaaa1111"]

            depth = len(app.screen_stack)
            home.on_drill_table_picked(type("M", (), {"node": pnode})())   # group row → Picked is ignored
            await pilot.pause()
            assert len(app.screen_stack) == depth                 # project row opens nothing
...
def test_home_reorders_live_by_recent_activity(monkeypatch, tmp_path):
    # The request: Home's session order must track most-recent activity live (in the poll), not freeze
    # at mount. Bump a session's mtime + add a brand-new one, poll once, and assert the order flips and
    # the newcomer joins — while the user's project expand and the cursor follow their session. We
    # assert RELATIONSHIPS (who sits above whom, what survives) — never absolute rows or token numbers.
    from cost_xray import tui
    ...
    async def go():
        ...
        async with app.run_test() as pilot:
            ...
            home = app.screen
            assert isinstance(home, HomeScreen)
            assert order(home) == ["bbbbbbbb2222", "aaaaaaaa1111"]   # mount: bravo (3) over alpha (2)

            # user opens the project and parks the cursor on the (currently-bottom) alpha
            ...
            # alpha becomes the most-recently-active and a brand-new session shows up
            ...
            home._load()                                             # the poll path (worker → _sync)
            await app.workers.wait_for_complete()
            await pilot.pause()

            assert order(home) == ["aaaaaaaa1111", "cccccccc3333", "bbbbbbbb2222"]   # 9 > 5 > 3
            assert home._proj_nodes[pkey].expanded                   # expand survived the re-sort
            cur = home._table._visible()[home._table.cursor][0]
            assert cur is home._sess_nodes["aaaaaaaa1111"]           # cursor followed alpha as it rose

    asyncio.run(go())


def test_home_numeric_columns_align_across_depths(monkeypatch, tmp_path):
    # The request: Home columns stay aligned no matter how many sub-rows are open. Expand the project
    # so the agent (depth 0), project (depth 1) and session (depth 2) rows all show — with deliberately
    # uneven label lengths — render the one table, and assert the cost amount sits at the SAME column
    # on every row. Indent and label length only move column 0; the numeric columns don't budge.
    ...
    async def go():
        ...
        async with app.run_test() as pilot:
            await app.workers.wait_for_complete()
            home = app.screen
            home._proj_nodes[("claude", "/home/u/web")].expanded = True   # open the leaves
            home._table.refresh()
            await pilot.pause()
            import io
            cons = Console(record=True, width=100, file=io.StringIO())   # record + sink: plain text, no stdout
            cons.print(home._table.render())
            text = cons.export_text()
            # the cost column header isn't "$", so every "$" is a dollar amount (rows + footer). All
            # bills are single-digit, so a constant start column ⇔ a constant right-justified column.
            offsets = {ln.index("$") for ln in text.splitlines() if "$" in ln}
            assert len(offsets) == 1            # one offset across agent + project + 2 sessions + footer
...
def test_context_table_drills_to_mcp_server(monkeypatch, tmp_path):
    ...
    async def go():
        ...
        async with app.run_test():
            ...
            servers = mcp.loader()                 # MCP category → server first
            assert [s.label for s in servers] == ["notion"]
            tools = servers[0].loader()            # server → its tools (full original names)
...
def test_skill_loads_render_end_to_end(monkeypatch, tmp_path):
    # True end-to-end: a raw capture (skills catalog in a system-role message + an injected
    # SKILL.md body + a Skill tool call) → materialize → the real DetailScreen renders a per-skill
    # "Skill loads" category in BOTH the cost and context tables, drilling to the skill name, while
    # the Skill *tool* call stays in the generic tool-I/O row.
    ...
    monkeypatch.setattr(tui, "_sessions", lambda: [])      # keep HomeScreen off the real ~/.cost-xray
    monkeypatch.setattr(tui, "_ensure_fresh", lambda d: None)
    ...
    async def go():
        ...
        async with app.run_test():
            ...
            # cost panel: Skill loads is its own Messages category, drilling per skill
            loads = screen._cost_nodes[("Messages", "Skill loads")]
            assert [n.label for n in loads.loader()] == ["ascii-banner"]
            # cost panel: the catalog drills Static Skills per skill
            skills = {n.label for n in screen._cost_nodes[("Static", "Skills")].loader()}
            assert {"ascii-banner", "json-sort-keys"} <= skills
            # the Skill *tool* call stays generic — not a per-skill category
            assert ("Messages", "system tool use+output") in screen._cost_nodes

            cons = Console(width=100)                       # it actually renders on screen
            with cons.capture() as cap:
                cons.print(screen._cost.render())
            assert "Skill loads" in cap.get()

            # context panel: same per-skill Skill loads on this turn's window
...
def test_drilltable_windows_and_follows_cursor():
    # With more rows than fit, the table renders a window that scrolls to keep the cursor visible
    # (the fix for "expanding pushes children out of the frame").
    ...
    async def go():
        ...
        async with app.run_test(size=(60, 12)) as pilot:
            app.t.focus()
            await pilot.pause()
            assert app.t._page < 40                       # window smaller than the row count
            for _ in range(30):
                await pilot.press("down")
            assert app.t.cursor == 30
            assert app.t._offset > 0                       # scrolled down
            assert app.t._offset <= app.t.cursor <= app.t._offset + app.t._page - 1   # cursor on screen
...
def _click(table, y):
    # The DrillTable only reads `event.y`; mirror the file's fake-event style (see on_drill_table_picked).
    table.on_click(type("Click", (), {"y": y})())


def test_drilltable_click_drills_like_keyboard():
    # Clicking a row moves the cursor to it and drills it — a parent toggles open/closed, a leaf posts
    # Picked — so the mouse reaches the same drill the keyboard does (Detail now matches Home's tree).
    from textual.app import App
    ...
    async def go():
        ...
        async with app.run_test(size=(60, 20)) as pilot:
            app.t.focus()
            await pilot.pause()
            # body starts two rows below the panel top (border + header), so y=2 is the first row.
            _click(app.t, 2)
            await pilot.pause()
            assert app.t.cursor == 0 and app.t.roots[0].expanded is True   # parent → open
            _click(app.t, 2)
            await pilot.pause()
            assert app.t.cursor == 0 and app.t.roots[0].expanded is False  # same click → closed
            _click(app.t, 3)                                               # next visible row → open it
            await pilot.pause()
            assert app.t.cursor == 1 and app.t.roots[1].expanded is True
            # its child leaf is now the next visible row (row0, row1, leaf …) → click it → Picked
            _click(app.t, 4)
            await pilot.pause()
            assert picked and picked[-1].label == "leaf"
            # clicking the chrome (header / above the body) drills nothing
            before = app.t.cursor
            _click(app.t, 1)
            await pilot.pause()
...
def test_drilltable_wheel_scrolls_window():
    # No scrollbar (the table windows internally), so the wheel moves the cursor, which drags the
    # window — wheel up returns to where it started.
    from textual.app import App
